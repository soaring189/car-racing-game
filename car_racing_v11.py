# This version of the game improves the display of scores and speed indicators.
import random
import pygame as p
import sys
p.init()
...
for i in range(0, 10):
    ...
    modified_number = p.transform.smoothscale(p.image.load(f"./Assets/{i}.png").convert_alpha(), (48, 72))
    number_list.append(modified_number)
m_image = p.transform.smoothscale(p.image.load(f"./Assets/m.png").convert_alpha(), (48, 48))
...
circle_image = p.transform.smoothscale(p.image.load("./Assets/circle.png").convert_alpha(), (10, 10))
...
def display_stats(speed, max_speed):
    ...
    for n in range(5):
        game_screen.blit(number_list[(int(score) // (10 ** n)) % 10], (626, 475 - (n * 100)))
